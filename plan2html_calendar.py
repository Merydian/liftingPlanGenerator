...
class plan2html:
    ...
    def rpe(self, exercise, rpe, sets, reps, week, ov, roundval=2.5, accessory=False):
        ...
        if week % self.deload == 0:
            sets = round(sets / 2)
        if accessory:
            return f'{exercise.title()}:linebreak{sets}x{reps}xX @{rpe}linebreaklinebreak'
        else:
            return f'bold{exercise.title()}:boldendlinebreak{sets}x{reps}x{str(roundval * round(((self.maxs[exercise] * self.lut.loc[rpe][reps])*0.95) / roundval) + ov)}linebreaklinebreak'

    def ueberblick(self):
        header = ["Name", "Height [cm]", 'Weight [Kg]', 'Age', 'BMR [KCal]', 'Protein per day [g]', 'BMI', 'Bi-Weekly overload [Kg]']
        main = [self.name, str(self.h), str(self.m), self.a, self.bmr, self.pro, self.bmi[0], self.ov]

        return tabulate([header, main], tablefmt='html')

    def amrap_test(self):
        header = [''] + [i.title() for i in list(self.test.keys())]
        test = ['Test [Reps@Weight]'] + [', '.join(str(y) for y in self.test[i][::-1]) for i in self.test]
    ...
    def plan(self):
        if self.days == 2:
            ...
            for i in range(self.weeks):
                ...
                dayOne = [
                    ...
                    self.rpe('seated calve raises', 8, 5, 12, week, ov, accessory=True) +
                ...
                dayTwo = [
                    ...
                    self.rpe('seated calve raises', 5, 5, 12, week, ov, accessory=True) +
        ...
        if self.days == 3:
            ...
            for i in range(self.weeks):
                ...
                desc = ['[Sätze xlinebreakWiederholungen xlinebreakGewicht,linebreak@ RPE]']
                ...
                x = [[f'Week: {week}', 'Day 1', 'Day 2', 'Day 3'], desc + dayOne + dayTwo + dayThree]
                self.program += tabulate(x, tablefmt='html')
        ...
        if self.days == 4:
            ...
            for i in range(self.weeks):
                ...
                x = [[f'Week: {week}', 'Day 1', 'Day 2', 'Day 3', 'Day 4'], desc + dayOne + dayTwo + dayThree + dayFour]
                self.program += tabulate(x, tablefmt='html')

    def annotation(self):
        anno = lambda exercise, link: f'<a href="{link}">{exercise}</a>'

        annos = [anno('Benchpress', 'https://exrx.net/WeightExercises/PectoralSternal/BBBenchPress'),
                 anno('Cableflys', 'https://exrx.net/WeightExercises/PectoralSternal/CBStandingFly'),
                 anno('Lat Pulldown', 'https://exrx.net/WeightExercises/LatissimusDorsi/CBFrontPulldown'),
                 anno('Face Pulls', 'https://exrx.net/WeightExercises/DeltoidPosterior/CBStandingRearDeltRowRope'),
                 anno('Triceps Extensions', 'https://exrx.net/WeightExercises/Triceps/CBTriExt'),
                 anno('Overhead Press', 'https://exrx.net/WeightExercises/DeltoidAnterior/BBMilitaryPress'),
                 anno('Cable Row', 'https://exrx.net/WeightExercises/BackGeneral/CBStraightBackSeatedRow'),
                 anno('Skull Crusher', 'https://exrx.net/WeightExercises/Triceps/BBLyingTriExtSC'),
                 anno('Squats', 'https://exrx.net/WeightExercises/Quadriceps/BBSquat'),
                 anno('Hip Thrusts', 'https://exrx.net/WeightExercises/GluteusMaximus/BBHipThrust'),
                 anno('Legpress', 'https://exrx.net/WeightExercises/Quadriceps/LVSeatedLegPress'),
                 anno('Seated Calve Raise', 'https://exrx.net/WeightExercises/Soleus/LVSeatedCalfRaise'),
                 anno('Back Extensions', 'https://exrx.net/WeightExercises/ErectorSpinae/WtBackExtension'),
                 anno('Seated Leg Curls', 'https://exrx.net/WeightExercises/Hamstrings/LVSeatedLegCurl'),
                 anno('Lever Chest Press', 'https://exrx.net/WeightExercises/PectoralSternal/LVChestPressS')
                 ]
    ...
    def combine(self):
        ...
        with (open(name, 'w') as file):
            text = self.table_style() + ueberblick + self.ueberblick() + test + self.amrap_test() + plan + self.program + uebungen + self.annotation()
            text = text.replace('linebreak', '<br>').replace('boldend', '</b>').replace('bold', '<b>')
    ...
    def calendar(self):
        dd = pd.DataFrame(columns=['Subject','Start Date','Start Time','End Date','End Time','All Day Event','Description','Private'])
        ...
        for week in df:
            for i in range(self.days):
                ...
                dd = dd._append({'Subject': 'Eurotraining',
